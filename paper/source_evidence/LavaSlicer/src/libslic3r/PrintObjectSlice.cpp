...
static bool lavaslicer_is_injection_only_object(const PrintObject &object)
{
    const Print *print = object.print();
    if (!print || !print->config().inject_mode.value)
        return false;
    ...
    bool has_printable_model_part = false;
    for (const ModelVolume *vol : model_obj->volumes) {
        if (!vol->is_model_part())
            continue;
        has_model_part = true;
        if (vol->name != inject_name)
            has_printable_model_part = true;
    }

    return has_model_part && !has_printable_model_part;
}
...
void PrintObject::slice()
{
    if (! this->set_started(posSlice))
        return;
    if (lavaslicer_is_injection_only_object(*this)) {
        this->clear_layers();
    ...
    if (m_layers.empty() && lavaslicer_is_injection_only_object(*this)) {
        this->set_done(posSlice);
        return;
    }
...
void PrintObject::slice_volumes()
{
    ...
    if (!inject_skip.empty()) {
        const bool skip_entire_object = print->config().inject_mode.value && this->model_object()->name == inject_skip;
        if (skip_entire_object) {
            slicing_volumes.clear();
        } else {
