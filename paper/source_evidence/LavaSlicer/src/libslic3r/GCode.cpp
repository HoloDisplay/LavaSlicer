    ...
    if (print.config().inject_mode.value) {
        ...
        for (const PrintObject *obj : print.objects()) {
            ...
            for (const ModelVolume *vol : model_obj->volumes) {
                ...
                const Transform3d &vol_trafo = vol->get_matrix();
            ...
            // Use the highest printed layer Z as the rim (top of the mold)
            double top_z = m_max_layer_z;
            ...
            // Prime and inject while rising back up to the mold rim. This makes the
            // injection visible in preview and keeps the nozzle closer to the fill front.
            file.write("G1 E0.8 F2100             ; prime\n");
            file.write_format("; --- inject %.0f mm^3 while rising %.1fmm ---\n", volume_to_inject, plunge_depth);
            double current_z = plunge_z;
            const double z_span = std::max(0.0, rim_z - plunge_z);
            for (int s = 0; s < nseg; ++s) {
                const double next_z = z_span > 0.0 ? plunge_z + z_span * (double(s + 1) / double(nseg)) : plunge_z;
                const double z_delta = next_z - current_z;
                if (z_delta > 0.0 && e_seg > 0.0) {
                    // Feedrate is the Z motion speed needed to preserve the requested
                    // filament volumetric flow while the E axis is synchronized to Z.
                    const double z_feed = std::max(1.0, z_delta * feed / e_seg);
                    file.write_format("G1 Z%.3f E%.3f F%.0f\n", next_z, e_seg, z_feed);
                } else {
